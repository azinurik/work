...
# текущий момент в UTC (локальная полночь - offset)
curr_utc = curr_dt_local - curr_offset

# чтобы правильно выбрать "ближайший" день рождения,
# переведём текущий момент в таймзону рождения
curr_in_birth_tz = curr_utc + birth_offset
year = curr_in_birth_tz.year

# кандидат: день рождения в этом году (в tz рождения)
bday_local = birthday_local_for_year(year, birth_month, birth_day)
bday_utc = bday_local - birth_offset

# если уже прошёл относительно текущего момента — берём следующий год
